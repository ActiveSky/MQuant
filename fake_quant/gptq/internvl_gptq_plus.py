...
@torch.no_grad()
def gptq_internvl_fwrd_visual_clip_conv1(
    ...
    conv1_gptq = GPTQConv(model.model.vision_model.embeddings.patch_embedding.module)
    ...
    handles = []
    handles.append(
        model.model.vision_model.embeddings.patch_embedding.module.register_forward_hook(
            add_batch()
        )
    )
...
@torch.no_grad()
def gptq_internvl_fwrd_visual_clip_resblocks(
    model, dataset, dev, dataset_name, args, quantizers
):
    """
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                quantizers["model.vision_model.encoder.layers.%d.%s" % (i, name)] = (
                    gptq[name].quantizer
                )
                gptq[name].free()
...
@torch.no_grad()
def gptq_internvl_fwrd_llm(model, dataset, dev, dataset_name, args, quantizers):
    """
    From GPTQ repo
...
@torch.no_grad()
def internvl_rtn_gptq_fwrd_plus(model, dataset, dev, dataset_name, args):
    """
    From GPTQ repo
    """
    ...
    if args.quant_visual_clip:
        if args.visual_w_rtn:
            ...
            gptq_internvl_fwrd_visual_clip_resblocks(
                model, dataset, dev, dataset_name, args, quantizers
            )
